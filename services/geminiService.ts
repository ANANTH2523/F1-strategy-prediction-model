...
import { RaceScenario, TelemetryData, StrategyAnalysis, LapSimulation, ChatMessage, TireCompound } from '../types';
  ...
  properties: {
    ...
    startingGrid: {
      ...
      items: {
        ...
        required: ['position', 'driver']
      },
      description: "All 20 drivers and their starting positions."
    },
    raceLaps: { type: Type.INTEGER, description: "Total number of laps for the race." }
  },
  required: ['track', 'weather', 'availableTires', 'startingGrid', 'raceLaps'],
};
                ...
                properties: {
                    ...
                    events: {
                        ...
                        items: {
                            type: Type.OBJECT,
                            properties: {
                                type: { type: Type.STRING, enum: ['PIT', 'OVERTAKE', 'FASTEST_LAP', 'INFO', 'DRS', 'VSC', 'MECHANICAL_ISSUE'] },
...
export async function generateTelemetryForScenario(scenario: RaceScenario): Promise<TelemetryData[]> {
    ...
    for (let i = 0; i < top5Drivers.length; i++) {
        ...
        if (i === 0) {
            ...
            prompt = `
                Based on the F1 race scenario at ${scenario.track} (${scenario.weather}, ${scenario.raceLaps} Laps), simulate detailed telemetry data for the first 15 laps for the lead driver: ${driver}.
                ...
                4.  Tire Degradation: After the fuel burn-off phase, lap times should start to degrade (get slower).
            ...
            prompt = `
                Based on the F1 race scenario at ${scenario.track}, simulate telemetry for the first 15 laps for the driver in P${driverPosition}: ${driver}.
                ...
                3.  Metric Consistency: All other metrics (fuelLoad, tireWear, etc.) must be consistent and realistic.
...
export async function generateRaceScenarioAndTelemetry(options: ScenarioGenerationOptions = {}): Promise<{ scenario: RaceScenario, telemetry: TelemetryData[] }> {
  ...
  let scenarioPrompt = `Generate a realistic and detailed F1 race scenario for the 2025 season.
  ...
  3.  Starting Grid: Create a full, plausible 20-driver starting grid for the 2025 season, reflecting likely team performance tiers. Top teams/drivers should be at the front.
  `;
...
export async function analyzeRaceStrategy(scenario: RaceScenario, telemetry: TelemetryData[]): Promise<StrategyAnalysis> {
  ...
  const prompt = `
    ...
    **REPORT REQUIREMENTS:**
    ...
    3.  **Key Strategic Factors:** Analyze the undercut/overcut potential at this specific track. Discuss how the weather forecast impacts both plans (e.g., "The threat of rain on lap 40 makes a one-stop risky...").
...
export async function simulateRace(scenario: RaceScenario, strategy: StrategyAnalysis): Promise<LapSimulation[]> {
    ...
    const pitStops = planA.stints.slice(0, -1).map(stint => `pits on lap ${stint.endLap} for ${planA.stints[planA.stints.indexOf(stint) + 1].tireCompound} tires`).join(', ');

    const prompt = `
    You are a sophisticated F1 race simulation engine. Your task is to generate a realistic and exciting lap-by-lap summary of a race.

    **RACE DETAILS:**
    - Track: ${scenario.track}
    - Total Laps: ${scenario.raceLaps}
    - Starting Grid: A full 20-car grid, with top drivers including ${scenario.startingGrid.slice(0, 5).map(d => d.driver).join(', ')}.
    ...
    **SIMULATION RULES & PRIORITIES:**
    1.  **Narrative Focus:** Focus the detailed events (overtakes, battles) on the top 10 positions to create a clear narrative. You can mention midfield action if it's significant.
    2.  **Strategic Adherence:** The lead driver, ${scenario.startingGrid[0].driver}, MUST pit according to their strategy. Other drivers will have their own logical strategies you must invent.
    3.  **Logical Events:**
    ...
    5.  **Consistency:** Maintain driver characteristics. A famously aggressive driver might attempt more overtakes, while a smooth driver might excel at tire management.

    **OUTPUT FORMAT:**
    For EACH lap from 1 to ${scenario.raceLaps}, provide the running order for the top 5 and list all key events. Return a single, valid JSON object with a key "simulation" containing an array of lap objects, strictly following the provided schema.
    `;
